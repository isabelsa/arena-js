...
class Arena {
  constructor(opts) {
    ...
    this.requestHandler =
      ...
      ((method, url, data) => {
        return method === 'get'
          ? this.axios.request({ method, url, params: data }).then(({ data }) => data)
  ...
  group(slug) {
    ...
    return {
      ...
      channels: (opts) => this._req('GET', 'groups/' + slug + '/channels', opts).then(pullObject('channels')),
    };
  }
  ...
  channel(slug, data) {
    ...
    return {
      ...
      connections: (opts) =>
        this._req('GET', 'channels/' + slug + '/connections', data, opts).then(pullObject('channels')),

      channels: (opts) => this._req('GET', 'channels/' + slug + '/channels', data, opts).then(pullObject('channels')),

      contents: (opts) => this._req('GET', 'channels/' + slug + '/contents', data, opts).then(pullObject('contents')),

      collaborators: (opts) =>
        this._req('GET', 'channels/' + slug + '/collaborators', data, opts).then(pullObject('users')),
  ...
  block(id, data) {
    return {
      ...
      channels: (opts) => this._req('GET', 'blocks/' + id + '/channels', data, opts).then(pullObject('channels')),
  ...
  user(id, data) {
    return {
      ...
      channels: (opts) => this._req('GET', 'users/' + id + '/channels', data, opts).then(pullObject('channels')),

      following: (opts) => this._req('GET', 'users/' + id + '/following', data, opts).then(pullObject('following')),

      followers: (opts) => this._req('GET', 'users/' + id + '/followers', data, opts).then(pullObject('users')),
    };
  }
  ...
  search(q, data) {
    return {
      ...
      users: (opts) => this._req('GET', 'search/users', { q }, data, opts).then(pullObject('users')),

      channels: (opts) => this._req('GET', 'search/channels', { q }, data, opts).then(pullObject('channels')),

      blocks: (opts) => this._req('GET', 'search/blocks', { q }, data, opts).then(pullObject('blocks')),
    };
  }
}
